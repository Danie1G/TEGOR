...
class AccountInvoice(models.Model):
                    ...
                    "invoice_line_ids": [
                        (
                            ...
                            0,
                            {
                                "product_id": product.id,
                                "account_id": account,
                                "name": _("Station: %s - Operation: %s")
                                % (rec.get("ClaveEstacion"), rec.get("FolioOperacion")),
                                "quantity": 1.0,
                                "product_uom_id": product.uom_id.id,
                                "tax_ids": taxes,
                                "price_unit": float(price),
    ...
    def _search_invoice(self, cfdi):
        ...
        uuid = self._l10n_mx_edi_decode_cfdi(etree.tostring(cfdi)).get("uuid")
        if serie_folio:
            domain.append("|")
    ...
    def _get_wrong_lines(self):
        ...
        if not result:
            return result
        return result.filtered(lambda l: not l.l10n_mx_edi_is_ecc)
